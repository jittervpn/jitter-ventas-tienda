...
export function checkEnv() {
  const missing = ["CF_API_TOKEN", "ZONE_NAME"].filter((k) => !clean(process.env[k]));
  return missing.length ? "Faltan variables de entorno en Vercel: " + missing.join(", ") : null;
}

// Limpia espacios, comillas y saltos de línea que se cuelan al pegar desde el móvil
export const clean = (v) => (v || "").trim().replace(/^["']|["']$/g, "").trim();

let zoneCache = null;
/** Devuelve el Zone ID: usa ZONE_ID si es válido; si no, lo busca por ZONE_NAME. */
export async function zoneId() {
  if (zoneCache) return zoneCache;
  const env = clean(process.env.ZONE_ID);
  const name = clean(process.env.ZONE_NAME);
  const zones = await cfRequest(`/zones?name=${encodeURIComponent(name)}`);
  if (!zones.length) throw new Error(`la zona "${name}" no existe en esta cuenta o el token no puede verla`);
  zoneCache = zones[0].id;
  if (env && env !== zoneCache) console.warn(`ZONE_ID en Vercel (${env}) no coincide con la zona real (${zoneCache}); usando la real`);
  return zoneCache;
}
...
export async function cfRequest(path, init = {}) {
  const res = await fetch(CF_API + path, {
    ...init,
    headers: { authorization: "Bearer " + clean(process.env.CF_API_TOKEN), "content-type": "application/json" },
  });
...
const statsName = () => `_jitterx-stats.${clean(process.env.ZONE_NAME)}`;

export async function getTotal() {
  const res = await cfRequest(`/zones/${await zoneId()}/dns_records?type=TXT&name=${statsName()}`);
  const txt = res[0];
...
export async function incrementTotal() {
  ...
  const zone = await zoneId();
  if (id) await cfRequest(`/zones/${zone}/dns_records/${id}`, { method: "PATCH", body });
  else await cfRequest(`/zones/${zone}/dns_records`, { method: "POST", body });
}
